...
export default function Navbar({ lang, content, setLang }: { lang: Language; content: any; setLang: (l: Language) => void }) {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${mobileOpen ? 'bg-white shadow-lg' : 'bg-white shadow-sm'} py-4 border-b border-gray-200`}>
      ...
      {mobileOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white shadow-xl p-6 space-y-4 border-t border-gray-200">
          <a 
            ...
            className="block text-gray-800 hover:text-primary-600 text-lg py-3 px-4 rounded-lg hover:bg-gray-50 transition-all hover:scale-[1.02]"
          >
            {content.services}
          </a>
          <a 
          ...
          <button className="group relative w-full bg-gradient-to-r from-primary-500 via-primary-600 to-primary-700 hover:from-primary-600 hover:via-primary-700 hover:to-primary-800 text-white py-4 rounded-full font-medium flex items-center justify-center gap-2 shadow-lg transition-all duration-300 transform hover:scale-[1.02] overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700"></div>
